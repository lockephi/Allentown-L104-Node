...
for i in range(N_CTRL3):
    ...
    ln_c = math.log(C)
    best = min(abs(ln_c - k * math.pi) for k in range(1, 5))
    ctrl_gaps[i] = best
...
p3 = gap_gc < 0.02 and gc_pctl3 > 99.5
proof_pass['03_log_2pi'] = p3
proof_times['03'] = time.time() - t0
if not p3:
    reason = []
    if gap_gc >= 0.02: reason.append(f'Gap {gap_gc:.6f} exceeds 0.02')
    if gc_pctl3 <= 99.5: reason.append(f'Percentile {gc_pctl3:.2f}% below 99.5%')
    print(f'     Reason: {"; ".join(reason)}')
print(f'\n  >> PROOF 3: {"PASS" if p3 else "FAIL"} ({proof_times["03"]:.3f}s)\n')


# ===========================================================================
# PROOF 4: PHI LATTICE MULTI-SCALE ALIGNMENT
# GC / phi^k produces clean integers at multiple scales
# PASS: >= 4 alignments with gap < 0.05 (vs < 2 expected random)
# ===========================================================================
print('--- PROOF 4: PHI LATTICE MULTI-SCALE ALIGNMENT ---')
print('  GC / phi^k produces clean integers at multiple scales')
print('  Pass if: >= 4 alignments with gap < 0.05 (vs < 2 expected random)')
t0 = time.time()
...
for k in range(-20, 21):
    ...
    nearest = round(val)
    gap = abs(val - nearest)
    if gap < 0.05 and 2 <= nearest <= 5000:
        alignments.append((k, nearest, gap))

print(f'  Found {len(alignments)} phi-lattice alignments (gap < 0.05):')
for k, n, g in alignments:
    notes = []
    pf = prime_factors(n) if n > 1 else set()
    if n == 326: notes.append("2*163 HEEGNER")
    if pf.issubset(MERSENNE_EXP) and len(pf) > 0:
        notes.append(f"Mersenne({'*'.join(str(f) for f in sorted(pf))})")
    if abs(math.sqrt(n) - round(math.sqrt(n))) < 0.01:
        notes.append("PERFECT SQUARE")
    note_str = f"  ({', '.join(notes)})" if notes else ""
    print(f'    GC/phi^{k:+3d} ~ {n:>5d}  (gap {g:.8f}){note_str}')
...
for _ in range(N_CTRL4):
    ...
    for k in range(-20, 21):
        val = C / (PHI ** k)
        if abs(val - round(val)) < 0.05 and 2 <= round(val) <= 5000:
            count += 1
    ctrl_align_counts.append(count)

ctrl4_arr = np.array(ctrl_align_counts)
print(f'\n  Control ({N_CTRL4:,} random):')
print(f'    Mean alignments: {float(np.mean(ctrl4_arr)):.2f}')
print(f'    Max alignments:  {int(np.max(ctrl4_arr))}')
print(f'    GOD_CODE:        {len(alignments)}')
gc_pctl4 = float(np.sum(ctrl4_arr < len(alignments))) / N_CTRL4 * 100
print(f'    Percentile:      {gc_pctl4:.1f}%')

p4 = len(alignments) >= 4
proof_pass['04_phi_lattice'] = p4
...
print('  Pass if: >= 2 large partial quotients (>=50) in first 15 terms')
t0 = time.time()
...
large_pqs = [(i, a) for i, a in enumerate(cf[1:15], 1) if a >= 50]
print(f'  Large PQs (>=50) in first 15: {len(large_pqs)}')
...
for _ in range(N_CTRL5):
    ...
    for _ in range(15):
        ...
        if fc < 1e-13:
            break
        xc = 1.0 / fc
    count_large = sum(1 for a in cf_c[1:15] if a >= 50)
...
print('  Pass if: |cos(GC)| > 0.95 AND percentile > 90% among 100K controls')
t0 = time.time()
...
p6 = abs(cos_gc) > 0.95 and gc_cos_pctl > 90
...
print('  Pass if: < 0.01% of 500K controls share ALL four')
t0 = time.time()
...
p7 = gc_has_all and pct_all < 0.01
...
try:
    ...
    r_vqe = engine.vqe_optimize(cost_matrix=cost_vec, num_qubits=4, max_iterations=150)
    vqe_err = r_vqe['energy_error']
    vqe_ok = vqe_err < 0.05
    if vqe_ok: algo_pass += 1
    print(f'  VQE:    E_exact={r_vqe["exact_ground_energy"]:.6f} E_vqe={r_vqe["optimized_energy"]:.6f} err={vqe_err:.4f} {"OK" if vqe_ok else "FAIL"}')

    # 2. Grover
    r_grover = engine.grover_search(target=7, num_qubits=4)
