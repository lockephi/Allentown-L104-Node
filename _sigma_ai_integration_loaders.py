...
try:
    ...
    print(f"⚠ L104 import error (non-blocking): {type(e).__name__}: {e}")
    import traceback
    traceback.print_exc()
    IMPORTS_OK = False
...
def load_subsystem(subsystem_name: str) -> Dict[str, Any]:
    ...
    # Normalize subsystem name
    subsystem_name_normalized = subsystem_name.upper()
    if subsystem_name_normalized in ["NUMERICAL", "NUM"]:
        subsystem_name_normalized = "NUMERICAL"
    elif subsystem_name_normalized in ["SCIENCE", "SCI"]:
        subsystem_name_normalized = "SCIENCE"

    print(f"\n[LOADING] {subsystem_name_normalized} Subsystem")
    print("─" * 70)

    if subsystem_name_normalized == "ASI":
        loader = ASIKnowledgeBaseLoader()
        if loader.initialize():
            loader.load_facts()
            return loader.verify_integration()
        else:
            return {"status": "failed", "subsystem": subsystem_name_normalized}

    elif subsystem_name_normalized == "ML":
    ...
    elif subsystem_name_normalized == "NUMERICAL":
    ...
    elif subsystem_name_normalized == "SCIENCE":
        ...
        return {"status": "unknown", "subsystem": subsystem_name_normalized}
...
def main():
    ...
    # Get subsystems to load from command line
    if len(sys.argv) > 1:
        subsystems_to_load = sys.argv[1:]
    else:
        subsystems_to_load = ["ASI", "ML", "NUMERICAL", "SCIENCE"]

    # Normalize subsystem names
    normalized = []
    for s in subsystems_to_load:
        su = s.upper()
        if su in ["ASI", "ML", "NUMERICAL", "SCIENCE"]:
            normalized.append(su)
        elif su in ["NUM", "NUMER"]:
            normalized.append("NUMERICAL")
        elif su in ["SCI"]:
            normalized.append("SCIENCE")

    subsystems_to_load = normalized if normalized else ["ASI", "ML", "NUMERICAL", "SCIENCE"]
    ...
    for subsystem in subsystems_to_load:
        ...
        if status == "success":
            ...
            print(f"  ✗ {subsystem}: Unknown status")
    ...
    for subsystem, result in results["subsystems"].items():
        if result.get("status") == "success":
            print(f"  ✓ {subsystem:<12} integration_complete={result.get('integration_complete', False)}")
