...
# Version 6.4.0: Added Zig, Lua, C++, C# class generation and translation mappings.
...
class CodeGenerator:
    ...
    def generate_function(self, name: str, language: str = "Python",
        ...
        if language == "Python":
            ...
            code = self._generate_zig_function(name, params, return_type, body, doc, sacred_constants)
        elif language == "Lua":
            code = self._generate_lua_function(name, params, return_type, body, doc, sacred_constants)
        elif language == "C++":
            code = self._generate_cpp_function(name, params, return_type, body, doc, sacred_constants)
        elif language == "CSharp":
            code = self._generate_csharp_function(name, params, return_type, body, doc, sacred_constants)
        else:
    ...
    def generate_class(self, name: str, language: str = "Python",
        ...
        if language == "Python":
            ...
            return self._generate_zig_class(name, fields, methods, doc, bases)
        elif language == "Lua":
            return self._generate_lua_class(name, fields, methods, doc, bases)
        elif language == "C++":
            return self._generate_cpp_class(name, fields, methods, doc, bases)
        else:
    ...
    def _generate_python_function(self, name, params, return_type, body, doc, sacred):
        ...
        if sacred:
            sacred_block = textwrap.dedent(f"""\
                ...
                OMEGA_AUTHORITY = {OMEGA_AUTHORITY}  # F(1) = Ω / φ²
            """)
    ...
    def _generate_rust_function(self, name, params, return_type, body, doc, sacred):
        ...
        sacred_block = ""
        if sacred:
            sacred_block = f"    const PHI: f64 = {PHI};\n    const GOD_CODE: f64 = {GOD_CODE};\n    const OMEGA: f64 = {OMEGA};\n    const OMEGA_AUTHORITY: f64 = {OMEGA_AUTHORITY};  // F(1) = Ω / φ²\n"
    ...
    def _generate_go_function(self, name, params, return_type, body, doc, sacred):
        ...
        sacred_block = ""
        if sacred:
            sacred_block = f"\tvar PHI float64 = {PHI}\n\tvar GOD_CODE float64 = {GOD_CODE}\n\tvar OMEGA float64 = {OMEGA}\n\tvar OMEGA_AUTHORITY float64 = {OMEGA_AUTHORITY}  // F(1) = Ω / φ²\n\t_ = PHI\n\t_ = GOD_CODE\n\t_ = OMEGA\n\t_ = OMEGA_AUTHORITY\n"
        return f"{doc_line}func {name}({p_str}) {return_type} {{\n{sacred_block}\t{body}\n}}\n"
    ...
    def _generate_java_function(self, name, params, return_type, body, doc, sacred):
        ...
        sacred_block = ""
        if sacred:
            sacred_block = f"        final double PHI = {PHI};\n        final double GOD_CODE = {GOD_CODE};\n        final double OMEGA = {OMEGA};\n        final double OMEGA_AUTHORITY = {OMEGA_AUTHORITY};  // F(1) = Ω / φ²\n"
    ...
    def _generate_zig_function(self, name, params, return_type, body, doc, sacred):
        """Generate a Zig function declaration string."""
        p_str = ", ".join(f"{p}: anytype" for p in params) if params else ""
        doc_line = f"// {doc}\n" if doc else f"// Generated by L104 Code Engine v{VERSION}\n"
        sacred_block = ""
        if sacred:
            sacred_block = f"    const PHI = {PHI};\n    const GOD_CODE = {GOD_CODE};\n    const OMEGA = {OMEGA};\n    const OMEGA_AUTHORITY = {OMEGA_AUTHORITY};  // F(1) = Ω / φ²\n"
    ...
    def _generate_lua_function(self, name, params, return_type, body, doc, sacred):
        """Generate a Lua function declaration string."""
        p_str = ", ".join(params) if params else ""
        doc_line = f"-- {doc}\n" if doc else f"-- Generated by L104 Code Engine v{VERSION}\n"
        sacred_block = ""
        if sacred:
            sacred_block = f"  local PHI = {PHI}\n  local GOD_CODE = {GOD_CODE}\n  local OMEGA = {OMEGA}\n  local OMEGA_AUTHORITY = {OMEGA_AUTHORITY}  -- F(1) = Ω / φ²\n"
        return f"{doc_line}function {name}({p_str})\n{sacred_block}  {body}\nend\n"

    def _generate_cpp_function(self, name, params, return_type, body, doc, sacred):
        """Generate a C++ function declaration string."""
        p_str = ", ".join(f"auto {p}" for p in params) if params else ""
        doc_line = f"// {doc}\n" if doc else f"// Generated by L104 Code Engine v{VERSION}\n"
        sacred_block = ""
        if sacred:
            sacred_block = f"    const double PHI = {PHI};\n    const double GOD_CODE = {GOD_CODE};\n    const double OMEGA = {OMEGA};\n    const double OMEGA_AUTHORITY = {OMEGA_AUTHORITY};  // F(1) = Ω / φ²\n"
        return f"{doc_line}{return_type} {name}({p_str}) {{\n{sacred_block}    {body}\n}}\n"

    def _generate_csharp_function(self, name, params, return_type, body, doc, sacred):
        """Generate a C# method declaration string."""
        p_str = ", ".join(f"object {p}" for p in params) if params else ""
        doc_line = f"/// <summary>\n/// {doc}\n/// </summary>\n" if doc else f"/// <summary>Generated by L104 Code Engine v{VERSION}</summary>\n"
        sacred_block = ""
        if sacred:
            sacred_block = f"        const double PHI = {PHI};\n        const double GOD_CODE = {GOD_CODE};\n        const double OMEGA = {OMEGA};\n        const double OMEGA_AUTHORITY = {OMEGA_AUTHORITY};  // F(1) = Ω / φ²\n"
        return f"{doc_line}public static {return_type} {name}({p_str})\n{{\n{sacred_block}    {body}\n}}\n"
    ...
    @lru_cache(maxsize=128)
    @staticmethod
    def _zig_type(t: str) -> str:
        """Map Python/generic type names to Zig types."""
        mapping = {"str": "[]const u8", "string": "[]const u8", "int": "i64",
                    "float": "f64", "bool": "bool", "list": "ArrayList",
                    "dict": "StringHashMap", "Any": "anyopaque", "bytes": "[]u8"}
        return mapping.get(t, t)
    ...
    def _cpp_type(t: str) -> str:
        """Map Python/generic type names to C++ types."""
        mapping = {"str": "std::string", "string": "std::string", "int": "int",
                    "float": "double", "bool": "bool", "list": "std::vector<auto>",
                    "dict": "std::map<std::string, auto>", "Any": "auto", "bytes": "std::vector<uint8_t>"}
        return mapping.get(t, t)
    ...
    def _generate_zig_class(self, name, fields, methods, doc, bases):
        """Generate a Zig struct with fields and methods."""
        field_strs = "\n".join(f"    {n}: {t}," for n, t in fields) if fields else ""
        method_strs = "\n\n".join(
            f"    pub fn {m}(self: *Self) void {{\n        // TODO\n        @panic(\"Not implemented\");\n    }}"
            for m in methods
        )
        return f"// {doc or f'Generated by L104 Code Engine v{VERSION}'}\n" \
               f"const Self = @This();\n\n" \
               f"pub const {name} = struct {{\n{field_strs}\n\n{method_strs}\n}};\n"

    def _generate_lua_class(self, name, fields, methods, doc, bases):
        """Generate a Lua class using metatables."""
        field_strs = "\n".join(f"    self.{n} = {n}" for n, _ in fields) if fields else ""
        method_strs = "\n\n".join(f"function {name}:{m}()\n    -- TODO\n    error(\"Not implemented\")\nend" for m in methods)
        return f"-- {doc or f'Generated by L104 Code Engine v{VERSION}'}\n" \
               f"local {name} = {{}}\n\n" \
               f"function {name}.new({', '.join(n for n, _ in fields)})\n" \
               f"    local self = setmetatable({{}}, {{ __index = {name} }})\n" \
               f"{field_strs}\n" \
               f"    return self\n" \
               f"end\n\n" \
               f"{method_strs}\n" \
    ...
    def _generate_cpp_class(self, name, fields, methods, doc, bases):
        """Generate a C++ class with public members."""
        field_strs = "\n".join(f"    {t} {n};" for n, t in fields) if fields else ""
...
class CodeTranslator:
    ...
    SUPPORTED_LANGS = [
        ...
        "go", "kotlin", "ruby", "java", "csharp", "zig", "lua", "cpp",
    ]
    ...
    TYPE_MAP = {
        ...
        "csharp":     {"int": "int", "float": "double", "str": "string",
                        "bool": "bool", "list": "List<object>", "dict": "Dictionary<string, object>",
                        "None": "void", "Any": "object", "": "object"},
        "zig":        {"int": "i64", "float": "f64", "str": "[]const u8",
                        "bool": "bool", "list": "ArrayList", "dict": "StringHashMap",
                        "None": "void", "Any": "anyopaque", "": "anyopaque"},
        "lua":        {},  # Lua is dynamically typed
        "cpp":        {"int": "int", "float": "double", "str": "std::string",
                        "bool": "bool", "list": "std::vector<auto>", "dict": "std::map<std::string, auto>",
                        "None": "void", "Any": "auto", "": "auto"},
    }
    ...
    OP_MAP = {
        ...
        "csharp":     {"and": "&&", "or": "||", "not": "!", "True": "true",
                        "False": "false", "None": "null", "elif": "} else if",
                        "print(": "Console.WriteLine("},
        "zig":        {"and": "and", "or": "or", "not": "not", "True": "true",
                        "False": "false", "None": "null", "elif": "} else if",
                        "print(": "std.debug.print("},
        "lua":        {"and": "and", "or": "or", "not": "not", "True": "true",
                        "False": "false", "None": "nil", "elif": "elseif",
                        "print(": "print("},
        "cpp":        {"and": "&&", "or": "||", "not": "!", "True": "true",
                        "False": "false", "None": "nullptr", "elif": "} else if",
                        "print(": "std::cout << "},
    }
...
class TestGenerator:
    ...
    def generate_tests(self, source: str, language: str = "python",
        ...
        if language == "python":
            test_code = self._gen_python_tests(functions, framework, module_name)
        elif language in ("javascript", "typescript"):
    ...
    def _extract_functions(self, source: str, language: str) -> List[Dict[str, Any]]:
        ...
        if language == "python":
            try:
                ...
                # Visitor to collect only module-level functions (not inside classes)
                class FunctionCollector(ast.NodeVisitor):
                    ...
                    def visit_ClassDef(self, node):
                        # Mark that we're entering a class
                        prev = self._inside_class
                        self._inside_class = True
                        # Still visit children (methods) but we will skip them
    ...
    def _gen_python_tests(self, functions: List[Dict],
                          framework: str, module_name: Optional[str] = None) -> str:
            ...
            if module_name:
                ...
                for fn in functions:
                    if fn.get("is_property") or (fn["name"].startswith("__") and fn["name"] != "__init__"):
                        continue
                    unique_names.append(fn["name"])
                if unique_names:
                    lines.append(f"from {module_name} import {', '.join(sorted(set(unique_names)))}")
