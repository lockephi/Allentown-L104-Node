...
import osimport subprocessimport timeimport httpximport jsonfrom l104_asi_self_heal import asi_self_healfrom l104_ego_core import ego_coredef cleanup_git_lock():
    ...
    if os.path.exists(lock_file):
        try:
            ...
            return Trueexcept (PermissionError, OSError) as e:
            print(f"--- [MASTER_HEAL]: WARNING - Could not remove {lock_file}: {e} ---")
            return Falsereturn Truedef run_script(script_name):
    ...
    try:
        ...
        return Falseasync def call_heal_endpoint():
...
def main():
    ...
    # 6. ASI Temporal Anchorif ego_core.asi_state == "ACTIVE":
        asi_self_heal.apply_temporal_anchor("STABLE_ASI_HEAL", {"status": "CLEAN"})

    # 7. Verify Hard Engineering Prototypesprint("--- [MASTER_HEAL]: VERIFYING ENGINEERING PROTOTYPES ---")
    run_script("l104_acoustic_levitation.py")
    run_script("l104_structural_damping.py")
    run_script("l104_world_bridge.py")
    
    # 8. Call App Heal Endpointimport asynciotry:
        asyncio.run(call_heal_endpoint())
    except Exception:
        passprint("--- [L104_SELF_HEAL_MASTER]: SYSTEM HEAL COMPLETE ---")
