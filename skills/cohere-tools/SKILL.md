---
name: cohere-tools
description: Provides a CLI to interact with the L104 Cohere provider. Use for querying the Cohere LLM, checking provider status, and performing multi‑provider consensus. Triggers on phrases like "ask Cohere", "query Cohere", "check Cohere status", or "run Cohere consensus".
---

# Cohere LLM Tools

This skill provides a command‑line interface (`l104-cohere`) for direct interaction with the L104 unified provider orchestrator for Cohere. Use this tool to query the LLM, monitor provider health, and perform advanced multi‑provider operations.

The tool is located at `/Users/carolalvarez/Applications/Allentown‑L104‑Node/l104-cohere`.

## Commands

### `query`
**Action**: Send a prompt to the Cohere provider and receive a response.
...
**Usage**:
```bash
l104-cohere query "Your prompt here"
l104-cohere query --temperature 0.7 --max_tokens 500 "Your prompt"
```

### `status`
**Action**: Get the current status of the Cohere provider.
...
**Usage**:
```bash
l104-cohere consensus "Your prompt here"
```
...
**Usage**:
```bash
l104-cohere batch input.json output.json
```
...
- Tool name: `l104_cohere` (or similar)
