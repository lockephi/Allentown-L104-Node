---
name: groq-tools
description: Provides a CLI to interact with the L104 Groq provider. Use for querying the Groq LLM, checking provider status, and performing multi‑provider consensus. Triggers on phrases like "ask Groq", "query Groq", "check Groq status", or "run Groq consensus".
---

# Groq LLM Tools

This skill provides a command‑line interface (`l104-groq`) for direct interaction with the L104 unified provider orchestrator for Groq. Use this tool to query the LLM, monitor provider health, and perform advanced multi‑provider operations.

The tool is located at `/Users/carolalvarez/Applications/Allentown‑L104‑Node/l104-groq`.

## Commands

### `query`
**Action**: Send a prompt to the Groq provider and receive a response.
...
**Usage**:
```bash
l104-groq query "Your prompt here"
l104-groq query --temperature 0.7 --max_tokens 500 "Your prompt"
```

### `status`
**Action**: Get the current status of the Groq provider.
...
**Usage**:
```bash
l104-groq consensus "Your prompt here"
```
...
**Usage**:
```bash
l104-groq batch input.json output.json
```
...
- Tool name: `l104_groq` (or similar)
