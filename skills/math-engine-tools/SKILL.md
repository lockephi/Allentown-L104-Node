---
name: math-engine-tools
description: Provides a CLI to interact with the L104 Math Engine. Use for direct engine operations, status checks, cross‑engine validation, and unified analysis. Triggers on phrases like "run Math Engine analysis", "check Math Engine status", "validate with Math Engine", or "cross‑engine".
---

# Math Engine Tools

This skill provides a command‑line interface (`l104-math`) for direct interaction with the L104 Math Engine. Use this tool to perform engine‑specific operations, retrieve status, and execute cross‑engine workflows.

The tool is located at `/Users/carolalvarez/Applications/Allentown‑L104‑Node/l104-math`.

## Commands

### `status`
**Action**: Get the current status of the Math Engine.
...
### `analyze`
**Action**: Perform a unified analysis using the Math Engine.
...
**Usage**:
```bash
l104-math analyze --code "print('hello')" --science '{"entropy": 0.5}' --math '{"god_code_target": 1.618}'
l104-math analyze --text "Your input text"
```
...
**Usage**:
```bash
l104-math validate --input input.json
```
...
**Usage**:
```bash
l104-math cross‑engine --operation entropy_analysis
```
...
**Usage**:
```bash
l104-math simulate --type coherence_evolution
```
...
- Tool name: `l104_math-engine` (or similar)
