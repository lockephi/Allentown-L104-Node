---
name: openai-tools
description: Provides a CLI to interact with the L104 OpenAI GPT provider. Use for querying the OpenAI GPT LLM, checking provider status, and performing multi‑provider consensus. Triggers on phrases like "ask OpenAI GPT", "query OpenAI GPT", "check OpenAI GPT status", or "run OpenAI GPT consensus".
---

# OpenAI GPT LLM Tools

This skill provides a command‑line interface (`l104-openai`) for direct interaction with the L104 unified provider orchestrator for OpenAI GPT. Use this tool to query the LLM, monitor provider health, and perform advanced multi‑provider operations.

The tool is located at `/Users/carolalvarez/Applications/Allentown‑L104‑Node/l104-openai`.

## Commands

### `query`
**Action**: Send a prompt to the OpenAI GPT provider and receive a response.
...
**Usage**:
```bash
l104-openai query "Your prompt here"
l104-openai query --temperature 0.7 --max_tokens 500 "Your prompt"
```

### `status`
**Action**: Get the current status of the OpenAI GPT provider.
...
**Usage**:
```bash
l104-openai consensus "Your prompt here"
```
...
**Usage**:
```bash
l104-openai batch input.json output.json
```
...
- Tool name: `l104_openai` (or similar)
