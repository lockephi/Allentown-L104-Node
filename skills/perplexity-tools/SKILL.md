...
description: Provides a CLI to interact with the L104 Perplexity provider. Use for querying the Perplexity LLM, checking provider status, and performing multi‑provider consensus. Triggers on phrases like "ask Perplexity", "query Perplexity", "check Perplexity status", or "run Perplexity consensus".
---

# Perplexity LLM Tools

This skill provides a command‑line interface (`l104-perplexity`) for direct interaction with the L104 unified provider orchestrator for Perplexity. Use this tool to query the LLM, monitor provider health, and perform advanced multi‑provider operations.

The tool is located at `/Users/carolalvarez/Applications/Allentown‑L104‑Node/l104-perplexity`.

## Commands

### `query`
**Action**: Send a prompt to the Perplexity provider and receive a response.
...
**Usage**:
```bash
l104-perplexity query "Your prompt here"
l104-perplexity query --temperature 0.7 --max_tokens 500 "Your prompt"
```

### `status`
**Action**: Get the current status of the Perplexity provider.
- Shows availability, reliability score, last error (if any), and configuration details.
**Usage**:
```bash
l104-perplexity status
```
...
**Usage**:
```bash
l104-perplexity consensus "Your prompt here"
```
...
**Usage**:
```bash
l104-perplexity batch input.json output.json
```
...
- Tool name: `l104_perplexity` (or similar)
