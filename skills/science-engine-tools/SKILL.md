---
name: science-engine-tools
description: Provides a CLI to interact with the L104 Science Engine. Use for direct engine operations, status checks, cross‑engine validation, and unified analysis. Triggers on phrases like "run Science Engine analysis", "check Science Engine status", "validate with Science Engine", or "cross‑engine".
---

# Science Engine Tools

This skill provides a command‑line interface (`l104-science`) for direct interaction with the L104 Science Engine. Use this tool to perform engine‑specific operations, retrieve status, and execute cross‑engine workflows.

The tool is located at `/Users/carolalvarez/Applications/Allentown‑L104‑Node/l104-science`.

## Commands

### `status`
**Action**: Get the current status of the Science Engine.
...
### `analyze`
**Action**: Perform a unified analysis using the Science Engine.
...
**Usage**:
```bash
l104-science analyze --code "print('hello')" --science '{"entropy": 0.5}' --math '{"god_code_target": 1.618}'
l104-science analyze --text "Your input text"
```
...
**Usage**:
```bash
l104-science validate --input input.json
```
...
**Usage**:
```bash
l104-science cross‑engine --operation entropy_analysis
```
...
**Usage**:
```bash
l104-science simulate --type coherence_evolution
```
...
- Tool name: `l104_science-engine` (or similar)
